...
def mergesort3(lst: list) -> list:
    ...
    if len(lst) < 2:
        return lst[:]
    elif len(lst) == 2:  # Handle two-element lists directly.
        return [min(lst), max(lst)]
    else:
        third = len(lst) // 3
        left = mergesort3(lst[:third])
        middle = mergesort3(lst[third:2*third])
        right = mergesort3(lst[2*third:])
...
def merge3(lst1: list, lst2: list, lst3: list) -> list:
    ...
    if not lst1:
        return _merge(lst2, lst3)
    elif not lst2:
        return _merge(lst1, lst3)
    elif not lst3:
        return _merge(lst1, lst2)
    else:
        if lst1[0] <= lst2[0] and lst1[0] <= lst3[0]:
            return [lst1[0]] + merge3(lst1[1:], lst2, lst3)
        elif lst2[0] <= lst1[0] and lst2[0] <= lst3[0]:
            return [lst2[0]] + merge3(lst1, lst2[1:], lst3)
        else:
            return [lst3[0]] + merge3(lst1, lst2, lst3[1:])
