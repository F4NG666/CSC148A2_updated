...
def all_within_grid(coords: list[tuple[int, int]], n: int) -> bool:
    ...
    Preconditions:
    - n > 0
    >>> all_within_grid([(1, 2), (2, 2)], 3)
    True
    >>> all_within_grid([(1, 1), (3, 3)], 3)
    False
    """
    for coord in coords:
        for i in coord:
...
def reflect_vertically(coord: tuple[int, int], n: int) -> tuple[int, int]:
    ...
    - n > 0
    - within_grid(coord, n)
    >>> reflect_vertically((1, 2), 5)
    (3, 2)
    >>> reflect_vertically((0, 0), 4)
    (3, 0)
    """
    coord = (n - 1 - coord[0], coord[1])
    return coord
...
def reflect_points(line: list[tuple[int, int]],
    ...
    >>> reflect_points([(1, 2), (2, 3)], 5)
    [(3, 2), (2, 3)]
    >>> reflect_points([(0, 0), (4, 4)], 5)
    [(4, 0), (0, 4)]
    """
    reflected = []
    for row, column in line:
        reflected_coord = (n - 1 - row, column)  # Reflect the x-coordinate
        reflected.append(reflected_coord)
    return reflected
...
def is_row(squares: list[Square]) -> bool:
    ...
    for square in squares[1:]:
        if square.coord[0] != row or square.coord[1] - col != 1:
            return False
        col = square.coord[1]
    return True
...
def is_column(squares: list[Square]) -> bool:
    ...
    row, col = squares[0].coord
    for square in squares[1:]:
        if square.coord[1] != col or square.coord[0] - row != 1:
            return False
        row = square.coord[0]
    return True
...
def is_diagonal(squares: list[Square]) -> bool:
    ...
    False
    """
    if _is_diagonal(squares, True):
        return True
    if _is_diagonal(squares, False):
        return True
...
def _is_diagonal(squares: list[Square], up: bool) -> bool:
    ...
    for i in range(1, len(squares)):
        row = squares[i].coord[0] - start_square.coord[0]
        column = squares[i].coord[1] - start_square.coord[1]

        if up:
            if not (row == -i and column == i):
                return False
        else:
            if not (row == i and column == i):
                return False
...
class Line:
    ...
    Representation Invariants:
        ...
        - squares in the cell should always have different coordinates
        -the coordinate of squares in a cell should be sequential and consistent
        for instance, along a column coordinates always (Y,X+1)
    """
    cells: list[Square]
    ...
    def __init__(self, lst: list[Square]) -> None:
        ...
        RI violation caught!
        """
        if not (is_row(lst) or is_column(lst) or is_diagonal(lst)):
            raise AssertionError("RI violation caught!")
        self.cells = lst[:]
    ...
    def drop(self, item: str) -> int | None:  # | None:
        """
    ...
    def has_fiar(self, coord: tuple[int, int]) -> bool:
        ...
        """
        # TODO: Implement this method
        symbol_at_coord = None
        for square in self.cells:
            if square.coord == coord:
                symbol_at_coord = square.symbol
                break
        if symbol_at_coord is None:
            return False

        consecutive_count = 0
        for square in self.cells:
            if square.symbol == symbol_at_coord:
                consecutive_count += 1
...
def create_rows_and_columns(squares: list[list[Square]]) -> \
    ...
    columns = []
    num_columns = len(squares[0])
    for c in range(num_columns):
        column_squares = []
        for r in range(len(squares)):
            column_squares.append(squares[r][c])
        line = Line(column_squares)
        columns.append(line)
    return rows, columns
...
def create_mapping(squares: list[list[Square]]) -> \
    ...
    for r in range(n):
        for c in range(n):
            coord = (r, c)
            lines_at_coord = []
            lines_at_coord.append(rows[r])
            lines_at_coord.append(columns[c])
            for diag_line in diagonal_lines:
                if coord in [square.coord for square in diag_line.cells]:
                    lines_at_coord.append(diag_line)
            mapping[coord] = lines_at_coord
    return mapping
...
def get_all_down_diagonals(n: int) -> list[list[tuple[int, int]]]:
    ...
    for start in starts:
        down_diagonals.append(get_down_diagonal(start, n))
...
def get_coords_of_diagonals(n: int) -> list[list[tuple[int, int]]]:
    ...
    up_diagonals = []
    for down_diag in down_diagonals:
        up_diag = reflect_points(down_diag, n)
        up_diagonals.append(up_diag)
    al_diagonals = down_diagonals + up_diagonals
    return al_diagonals
...
def all_diagonals(squares: list[list[Square]]) -> list[Line]:
    ...
    """
    # TODO: Implement this function
    diagonal_coords = get_coords_of_diagonals(len(squares))
    diagonal_lines = []
    for diag_coord in diagonal_coords:
        diag_squares = [squares[r][c] for r, c in diag_coord]
        diagonal_lines.append(Line(diag_squares))
...
class Grid:
    ...
    def drop(self, col: int, item: str) -> int | None:
        ...
        if 0 <= col < self.n:
            for row in reversed(range(self.n)):
                if self._rows[row][col].symbol is None:
                    self._rows[row][col].symbol = item
                    return row
            return None
        else:
            raise ValueError()
    ...
    def has_fiar(self, coord: tuple[int, int]) -> bool:
        ...
        True
        """
        if not (0 <= coord[0] < self.n and 0 <= coord[1] < self.n):
            raise ValueError()

        target_symbol = self._rows[coord[0]][coord[1]].symbol
        if target_symbol is None:
            return False
        lines = self._mapping[coord]
        for line in lines:
            for i in range(len(line.cells) - 3):
                consecutive_count = 0
                for j in range(4):
                    if line.cells[i + j].symbol == target_symbol:
                        consecutive_count += 1
                    else:
                        break
                if consecutive_count == 4:
